...
class MLP {
    ...
    public:
        ...
        MLP(float lr_in, int inputLayer_in, int hiddenLayer_in, 
                ...
                int numSamples_in, float* y_in, int totalSamples_in) {
            totalSamples = totalSamples_in;
            ogDesign = designMatrix_in;
            lr = lr_in;
        ...
        void train() {
            for (int i = 0; i < iters; i++) {
                ...
                //increment offset of design matrix
                designMatrix += inputLayer * sizeof(float);
                if (designMatrix >= (ogDesign + (totalSamples * sizeof(float)))) {
                    designMatrix = ogDesign;
                }
            }
        }
...
double accuracy(float* yTrue, float* predictions[], int size) {
    double sum = 0;
    float* yPred = new float[size];
...
int main() {
    ...
    float* yDigit = new float[(int)(trainSize * numSamples)];
    float* predictions[NUM_CLASSES];
    for (int i = 0; i < NUM_CLASSES; i++) {
        for (int j = 0; j < (trainSize * numSamples); j++) {
            if (ytrain[j] == (1.0 * i)) {
                yDigit[j] = 1;
                // cout << "digit " << ytrain[j] << " is equal to " << i << endl;
            } else {
                // cout << "digit " << ytrain[j] << " is NOT equal to "<< i << endl;
                yDigit[j] = 0;
            }
        }
        MLP classifier = MLP(learningRate, numFeatures, numHidden, iters, Xtrain, numTrain, yDigit, trainSize * numSamples);
        cout << "training MLP " << i << "..." << endl;
        classifier.train(); 
        cout << "predicting MLP " << i << "..." << endl;
        float* prediction = classifier.predict(Xtest, numTest);
