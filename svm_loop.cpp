        ...
        void fit(double** X, int numFeatures, int* y, int size) { //self, X, y
            ...
            for (int i = 0; i < numIters; i++) {
                ...
                for (int j = 0; j < size; j++) {
                    if ((i % 100 == 0) && (j == 0)) {
                        printf("id = 0, i = %d, xVal = %f, cls[j] = %f\n", i, X[j][0], clsMap[j]);
                    }
                    // cout << "j = " << j << endl;
                    // bool constraint = satisfyConstraint(X[j], j);
                    bool constraint;
                    // double linearModel = dotProduct(x, w, lenW);
                    double linearModel = 0;
                    for (int k = 0; k < lenW; k++) {
                        linearModel += X[j][k] * w[k];
                    }
                    ...
                    if (constraint) {
                        for (int k = 0; k < lenW; k++) {
                            w[k] -= lr * w[k] * lambdaParam;
                            if ((k == 0) && (i % 100 == 0) && (j == 0)) {
                                printf("constrain: myW[id] = %f\n", w[k]);
                            }
                        } 
                        
                    // cout << "constrain for loop" << endl;
                    } else {
                        for (int k = 0; k < lenW; k++) {
                            w[k] -= lr * (lambdaParam * w[k] - (clsMap[j] * X[j][k]));
                            if ((k == 0) && (i % 100 == 0) && (j == 0)) {
                                printf("else constrain: myW[id] = %f\n", w[k]);
                            }
                        }
                    // cout << "constrain else for loop" << endl;
                    }
                }
            }
